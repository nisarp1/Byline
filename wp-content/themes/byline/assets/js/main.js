	...
	var NEWS = {
		...
		initGallery: function () {
			...
			function showSlide(index) {
				...
				$('.gallery-slider .gallery-slide').removeClass('active');
				$('.gallery-slider .gallery-slide').eq(index).addClass('active');
				currentSlide = index;
				updateCounter();
			}
			...
			window.openGallery = function(galleryId) {
				...
				// Find and clone only the slides for this specific gallery
				var gallerySlides = $('.gallery-slide[data-gallery="' + galleryId + '"]');
				
				// Filter out any slides that don't have proper content
				var validSlides = gallerySlides.filter(function() {
					var $slide = $(this);
					var $img = $slide.find('img');
					return $img.length > 0 && $img.attr('src') && $img.attr('src').trim() !== '';
				});
				
				totalSlides = validSlides.length;
				console.log('Total valid slides found for gallery', galleryId + ':', totalSlides);
				
				if (totalSlides === 0) {
					console.log('No valid slides found for gallery:', galleryId);
					return;
				}
				
				// Populate with only the valid slides for this gallery
				validSlides.each(function() {
					$('.gallery-slider').append($(this).clone());
				});
			...
			// Event listeners
			$('.gallery-container__type-2').on('click', function() {
				var galleryId = $(this).data('gallery');
				openGallery(galleryId);
			});
			
			// Also handle clicks on the gallery play button
			$('.gallery-play-btn').on('click', function(e) {
				e.stopPropagation();
				var galleryId = $(this).closest('.gallery-container__type-2').data('gallery');
				openGallery(galleryId);
			});
			
			// Handle gallery lightbox clicks
			...
			$(document).on('click', '#galleryModal', function(e) {
				console.log('Modal clicked:', e.target, this);
				if (e.target === this) {
					console.log('Closing gallery on background click');
					closeGallery();
				}
			});
