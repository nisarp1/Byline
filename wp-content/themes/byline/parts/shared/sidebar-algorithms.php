...
function simple_related_posts_fallback($content) {
    ...
    foreach ($recent_posts as $post) {
        ...
        $related_html .= '<h4 class="axil-post-title">';
...
function get_user_preferences() {
    ...
    if (!empty($view_history)) {
        ...
        $recent_views = array_slice($view_history, -10); // Last 10 views
        ...
        $preferences['favorite_categories'] = array_slice(array_keys($category_counts), 0, 5);
    }
    ...
    if (!empty($reading_history)) {
        ...
        $avg_reading_time = array_sum($reading_times) / count($reading_times);
        
        if ($avg_reading_time < 2) {
            $preferences['content_length_preference'] = 'short';
        } elseif ($avg_reading_time > 5) {
...
function get_recommended_posts($limit = 6, $exclude_ids = array()) {
    ...
    if (empty($user_preferences['favorite_categories'])) {
        return get_trending_posts($limit, $exclude_ids);
    }
    ...
    $recent_posts = get_posts(array(
        'post_type' => 'post',
        'post_status' => 'publish',
        'posts_per_page' => 50, // Get more posts to score
        'orderby' => 'date',
        'order' => 'DESC',
    ...
    foreach (array_slice($scored_posts, 0, $limit) as $scored_post) {
        $recommended_posts[] = $scored_post['post'];
    }
